...
# V3.3.9
class ArisBit(object):
    __uart_state = 0

    def __init__(self, car_type=1, com="/dev/arisbit", delay=.002, debug=False):
        # com = "COM30"
        # com="/dev/ttyTHS1"
        # com="/dev/ttyUSB0"
        # com="/dev/ttyAMA0"

        self.ser = serial.Serial(com, 921600)
        ...
        if self.ser.isOpen():
            print("Rosmaster Serial Opened! Baudrate=921600")
        else:
    ...
    # 设置电机数值倍率
    def SetMotorRatio(self,MotorRatio):
        self.MotorRatio = MotorRatio
    ...
    # def set_motor(self, speed_1, speed_2, speed_3, speed_4):
        ...
        #     pass
    # def SetMotorRPM(self,RPM1,RPM2,RPM3,RPM4):
    #     PPS1 = int(RPM1 * self.MotorRatio * self.MagneticPolePairs * 4 / 60)
    #     PPS2 = int(RPM2 * self.MotorRatio * self.MagneticPolePairs * 4 / 60)
    #     PPS3 = int(RPM3 * self.MotorRatio * self.MagneticPolePairs * 4 / 60)
    #     PPS4 = int(RPM4 * self.MotorRatio * self.MagneticPolePairs * 4 / 60)
        
    #     self.SetMotorPPS(PPS1, PPS2, PPS3, PPS4)
    #     print("SetMotorRPM:", RPM1, RPM2, RPM3, RPM4, "PPS:", PPS1, PPS2, PPS3, PPS4)

    # 设置电机转速
    def SetMotor(self, speed_1, speed_2, speed_3, speed_4):
        speed_1 = int(speed_1 * self.MotorRatio)
    ...
    def SetMotorPID(self, kp, ki, kd, forever=False):
        try:
            state = 0
            if forever:
                state = 0x5F

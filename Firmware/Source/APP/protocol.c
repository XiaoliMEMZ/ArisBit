...
void Upper_Data_Parse(uint8_t *data_buf, uint8_t num)
{
	...
	case FUNC_MOTOR:
	{
		// int16_t speed[4] = {0};
		// 解析16位有符号整数（小端序）
		int16_t motor_1 = (int16_t)(*(data_buf + 4) | (*(data_buf + 5) << 8));
		int16_t motor_2 = (int16_t)(*(data_buf + 6) | (*(data_buf + 7) << 8));
		int16_t motor_3 = (int16_t)(*(data_buf + 8) | (*(data_buf + 9) << 8));
		int16_t motor_4 = (int16_t)(*(data_buf + 10) | (*(data_buf + 11) << 8));
		...
		// 将[-1000, 1000]映射到PWM范围[-motor_pulse, motor_pulse]
		// speed[0] = (int16_t)motor_1 * (motor_pulse / 1000.0);
		// speed[1] = (int16_t)motor_2 * (motor_pulse / 1000.0);
		// speed[2] = (int16_t)motor_3 * (motor_pulse / 1000.0);
		// speed[3] = (int16_t)motor_4 * (motor_pulse / 1000.0);
		...
		{
			Motion_Set_Speed(motor_1, motor_2, motor_3, motor_4);
		}
		

		// speed[0] = (int16_t)motor_1;
		// speed[1] = (int16_t)motor_2;
		// speed[2] = (int16_t)motor_3;
		// speed[3] = (int16_t)motor_4;
		// // 控制小车运动，Motor_X=[-1000, 1000]。
		// Motion_Set_Speed(speed[0], speed[1], speed[2], speed[3]);
		break;
	}
